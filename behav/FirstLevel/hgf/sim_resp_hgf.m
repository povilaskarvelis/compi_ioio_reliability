function r = sim_resp_hgf(inputs, prc_model, prc_pvec, varargin)
%--------------------------------------------------------------------------
% This is the main function for simulating responses (or only perceptual 
% states) from a combination of perceptual and observation models, given 
...
%
%                        To ignore the input of a trial, code the input as 
%                        NaN. In this case, filtering is suspended for this 
%                        trial and all representations (i.e., inferences 
%                        on hidden states) will remain constant.
%
%                        Note that an input is often a composite event, for
%                        example a cue-stimulus contingency. If the agent 
%                        you are modeling is lerning such contingencies,
%                        inputs have to be coded in contingency space 
%                        (e.g., blue cue -> reward as well as green cue -> 
%                        no reward is coded as 1 while blue cue -> no 
%                        reward as well as green cue -> reward is coded as 
%                        0). The same applies to responses.
%
%                        If needed for a specific application, inputs can 
%                        be a matrix with further columns. The coding of 
%                        ignored trials described above then applies to its
%                        first column.
%
%     prc_model          Your choice of perceptual model. Choices are:
%
%                        - 'tapas_hgf_binary'  
%                        The binary Hierarchical Gaussian Filter (HGF) 
%                        model in the absence of perceptual uncertainty
%
%                        - 'tapas_hgf_binary3l'      
%                        The binary Hierarchical Gaussian Filter (HGF) 
%                        model,
...
try
    est = varargin{3}; % Get old model to extract n
    r.c_prc.n_levels = est.c_prc.n_levels; % Store levels (so that transp function can be used)
end

try
    est = varargin{3}; 
    r.c_prc.kaub = est.c_prc.kaub;
    r.c_prc.thub = est.c_prc.thub;
end
...
% try
%      prc_model = strrep(prc_model,'_config','');
% end


r.c_sim.prc_model = est.c_prc.model; %r.c_sim.prc_model = prc_model;

% Store perceptual parameters

prc_namep_fun = est.c_prc.transp_prc_fun; %prc_namep_fun = str2func([prc_model, '_transp']);

[~, r.p_prc]   = prc_namep_fun(r,prc_pvec);
r.p_prc.p = prc_pvec;
...
if nargin > 4
    ...
    try
        r.c_sim.obs_model = strrep(r.c_sim.obs_model,'_config','');
    end

    % Store observation parameters
    obs_pvec = varargin{2};
    obs_namep_fun = str2func([r.c_sim.obs_model, '_transp']);
    [~, r.p_obs]  = obs_namep_fun(r,obs_pvec);
    r.p_obs.p = obs_pvec;
    ...
    r.c_sim.seed = NaN;
    if nargin > 5
        r.c_sim.seed = varargin{4};
    end
    
    %----------------------------------------------------------------------
    % FIXME: This (below) needs to be adapted to also get RTs (when using a 
    % model that predicts RTs you need to create a switch and add the log 
    % RTs as output and store them in r.y(:,2)
    %----------------------------------------------------------------------
    
    % Obtain map choices
    [r.ch_sampled, r.prob, r.y] = obs_fun(r, infStates, r.p_obs.p);
