...
for j = 1:l-1
    ...
    subplot(l,1,j);
    xlim([0 ts(end)+1]);
    title(['Posterior expectation of x_' num2str(l-j+1)], 'FontWeight', 'bold', 'FontSize', 26);
    ylabel(['\mu_', num2str(l-j+1)], 'FontWeight', 'bold', 'Color', 'k', 'FontSize', 26);
...
%     end
% end
title(sprintf('Posterior expectation s(\\mu_2)'), 'FontSize', 26)
ylabel('u, s(\mu_2)', 'FontWeight', 'bold', 'Color', 'k', 'FontSize', 26);
xlabel('Trial number', 'FontWeight', 'bold', 'Color', 'k', 'FontSize', 26);
xlim([0 ts(end)+1]);
ylim([-0.05 1.05])
