function [X,Y,T,auc,optrocpt,subY,subYnames] = ...
    perfcurve(cls,scores,posClass,varargin)
...
if docv
    [ncvel,scores,cls,weights] = stackcv(scores,cls,weights);
    ncvcum = [0; cumsum(ncvel)];
end

% Get the true classes
trueNames = levels(cls);
...
function [scores,cls,weights,ncv] = preparedata(scores,cls,weights)
...
if iscell(scores) % Cross-validated scores and labels
    ...
    cls = cls(:);
    ltype = cellfun(@class,cls,'UniformOutput',false);
    for icv=2:ncv
    ...
    for icv=1:ncv
        if strcmp(ltype{1},'char')
            cls{icv} = cellstr(cls{icv});
        end
        cls{icv} = classreg.learning.internal.ClassLabel(cls{icv});
        nell(icv) = numel(cls{icv});
    end
    ...
    cls = classreg.learning.internal.ClassLabel(cls);
    nl = numel(cls);
    if ns~=nl
...
function [ncvel,scores,cls,weights] = stackcv(scores,cls,weights)
